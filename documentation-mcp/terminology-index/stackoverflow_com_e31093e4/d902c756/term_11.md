# DeprecationWarning

**Type:** TECHNICAL_TERM  
**Confidence:** 0.70  
**Source:** https://stackoverflow.com/questions/1319615/how-do-i-declare-custom-exceptions-in-modern-python

## Definition
No definition available

## Context
de will thank you. If you use the deprecated message attribute, assigning it yourself will avoid a `DeprecationWarning`:

```
class MyAppValueError(ValueError):
    '''Raise when a specific subset of values in context

## Metadata
```json
{
    "term_type": "TECHNICAL_TERM",
    "confidence": 0.7,
    "frequency": 4,
    "source_url": "https://stackoverflow.com/questions/1319615/how-do-i-declare-custom-exceptions-in-modern-python",
    "metadata": {'extraction_method': 'heuristic'}
}
```
