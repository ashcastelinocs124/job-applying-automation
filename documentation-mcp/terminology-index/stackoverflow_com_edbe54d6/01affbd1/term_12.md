# values

**Type:** TECHNICAL_TERM  
**Confidence:** 0.70  
**Source:** https://stackoverflow.com/questions/3294889/iterating-over-a-dictionary-using-a-for-loop-getting-keys

## Definition
```
d = {'x': 1, 'y': 2, 'z': 3}
```

Any time we iterate over it, we iterate over the keys

## Context
not just `for` loops. The important word here is "iterating".

A dictionary is a mapping of keys to values:

```
d = {'x': 1, 'y': 2, 'z': 3}
```

Any time we iterate over it, we iterate over the keys. The

## Metadata
```json
{
    "term_type": "TECHNICAL_TERM",
    "confidence": 0.7,
    "frequency": 4,
    "source_url": "https://stackoverflow.com/questions/3294889/iterating-over-a-dictionary-using-a-for-loop-getting-keys",
    "metadata": {'extraction_method': 'heuristic'}
}
```
